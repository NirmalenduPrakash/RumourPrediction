...
os.chdir('/home/svu/e0401988/NLP/BC_TREE/rumour prediction')
...
def train_loader(val_index):
  for e in [l for i,l in enumerate(event_labels) if i!=val_index]:
    with open('data/Encoding/BERT/data_UnlimitedTree_{}_balanced.pkl'.format(e),'rb') as f:
      ev=pickle.load(f)
    yield ev,e

def val_loader(val_index):
  # for e in [l for i,l in enumerate(event_labels) if i==val_index]:
  with open('data/Encoding/BERT/data_UnlimitedTree_{}.pkl'\
