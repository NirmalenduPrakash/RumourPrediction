...
Original file is located at
    https://colab.research.google.com/drive/1dAqL0KpFyZF--8wwiGEpHRo7Nl7wdsjT
"""
  ...
  for r in ['rumours']:
    ...
    for d in dirs:
      ...
      if(d=='.DS_Store'):
        continue
      sources.append(d)
      replies.extend(glob2.glob(root_dir+'/'+e+'/'+r+'/'+d + '/reactions/*.json'))
      # with open(root_dir+'/'+e+'/'+r+'/'+d+ '/annotation.json','r') as f:
      #   veracity=json.load(f)
      #   if(veracity is not None):
      #     veracity_dict.update({d:veracity_enum[veracity['is_rumour']]})
      #   else:
      #     print('annotation none for :{}'.format(d))       

# load stance labels
stance_dict={}
stance_annotation_file='en-scheme-annotations.json'
...
print(len(sources))
replies=[r.split('/')[-1].replace('.json','') for r in replies]
keys=list(stance_dict.keys())
...
# with open('rumoureval-subtaskA-train.json','r') as f:
#   old_stance_dict=json.load(f)
# len(old_stance_dict) 
...
veracity_dict={}
root_dir='semeval2017-task8-dataset/rumoureval-data'
replies=[]
sources=[]
for e in event_labels:
  ...
  for d in dirs:
    ...
    if(d=='.DS_Store'):
      continue
    sources.append(d)
    replies.extend(glob2.glob(root_dir+'/'+ e + '/' + d + '/replies/*.json'))      

# load stance labels
stance_dict={}
stance_annotation_file='rumoureval-subtaskA-train.json'
with open(stance_annotation_file,'r') as f:
  stance_dict=json.load(f)

# sources[0]
'544350712365207552' in keys

"""# Tree Structure"""

import zipfile
with zipfile.ZipFile('all-rnr-annotated-threads.zip', 'r') as zip_ref:
    zip_ref.extractall()
...
source={'id':source['id'],'text':embedding.embed(source['text'].split())} 
source
