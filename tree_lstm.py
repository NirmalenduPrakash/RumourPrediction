...
# load stance labels
stance_dict={}
stance_annotation_file='all-rnr-annotated-threads/stance-annotations.json'
...
#     ev.create_tree(gb)
#     with open('/content/drive/My Drive/PHEME/Encoding/{}/data_UnlimitedTree_{}.pkl'.format(typ,e),'wb') as f:
#       pickle.dump(ev,f)
...
#   ev_label_count=[]
#   for e in ['charliehebdo','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
#     with open('/content/drive/My Drive/PHEME/Encoding/{}/data_UnlimitedTree_{}.pkl'.format(typ,e),'rb') as f:
#       ev=pickle.load(f)
#     label={0:0,1:0,2:0}   
#     # for event in ev:
#     for tree in ev.treelist:
...
#       for i in range(260):
#         ev.treelist.append(random.sample(lbl_2_list,1)[0])    
#     with open('/content/drive/My Drive/PHEME/Encoding/{}/data_UnlimitedTree_{}_balanced.pkl'.format(typ,e),'wb') as f:
#       pickle.dump(ev,f) 

#   ev_label_count=[]
#   for e in ['charliehebdo','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
#     with open('/content/drive/My Drive/PHEME/Encoding/{}/data_UnlimitedTree_{}_balanced.pkl'.format(typ,e),'rb') as f:
#       ev=pickle.load(f)
#     label={0:0,1:0,2:0}   
#     # for event in ev:
#     for tree in ev.treelist:
...
def train_loader(val_index):
  for e in [l for i,l in enumerate(event_labels) if i!=val_index]:
    with open('/content/drive/My Drive/PHEME/Encoding/BERT/data_UnlimitedTree_{}_balanced.pkl'.format(e),'rb') as f:
      ev=pickle.load(f)
    yield ev,e

def val_loader(val_index):
  # for e in [l for i,l in enumerate(event_labels) if i==val_index]:
  with open('/content/drive/My Drive/PHEME/Encoding/BERT/data_UnlimitedTree_{}.pkl'\
            .format(event_labels[val_index]),'rb') as f:
    ev=pickle.load(f)
...
# model.load_state_dict(torch.load('/content/drive/My Drive/model_UnlimitedTree_BERT.pt'))
  ...
  for event,lbl in train_loader(val_index):
    ...
    for indx,tree in enumerate(event.treelist):
      ...
      for i in range(len(nodes)):
        ...
        nodes[i].c=None
      model.reset_h_c()  
      # print([(node.id,node.y) for node in nodes])
      for i in range(len(nodes)):
        if(i!=(len(nodes)-1)):
          result=model(nodes[i])
          # for few records annotation not available
          if(nodes[i].y is not None):
            stance_losses.append(criterion(result,torch.tensor(nodes[i].y).view(-1)))            
        else:
          result=model(nodes[i],is_last=True)
          # print(result,veracity_dict[str(tree[0])],nodes[i].id,nodes[i].children)                 
          loss+=criterion(result,torch.tensor(veracity_dict[str(tree[0])],dtype=torch.long).view(-1))         
      if(len(stance_losses)>0):
        loss+=sum(stance_losses)/len(stance_losses)
...
y_true=[]
y_pred=[]
for lbl in ['ottawashooting','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
