...
class TreeLSTMCell(nn.Module):
  def __init__(self,x_size,h_size):
    ...
    self.maxpool=nn.AdaptiveMaxPool1d(2400)
...
def balance_embedding(typ):
  ...
  for e in ['charliehebdo','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
    ...
    #   ev=pickle.load(f)
    files=glob2.glob('data/Encoding/{}/data_UnlimitedTree_{}_*.pkl'.format(typ,e))
    label={0:0,1:0,2:0}   
    # for event in ev:
    # for tree in ev.treelist:
    for file in files:
  ...
  for e in ['charliehebdo','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
    ...
    for file in files:
      with open(file,'rb') as f:
        tree=pickle.load(f)
      ev.treelist.append(tree) 
    with open('data/Encoding/{}/data_UnlimitedTree_{}.pkl'.format(typ,e),'wb') as f:
      pickle.dump(ev,f)       
    ...
    if(e=='charliehebdo'):
      ...
      for i in range(260):
        ev.treelist.append(random.sample(lbl_2_list,1)[0])    
    with open('data/Encoding/{}/data_UnlimitedTree_{}_balanced.pkl'.format(typ,e),'wb') as f:
      pickle.dump(ev,f) 

  ev_label_count=[]
  for e in ['charliehebdo','sydneysiege','germanwings-crash','ferguson','ottawashooting']:
    with open('data/Encoding/{}/data_UnlimitedTree_{}_balanced.pkl'.format(typ,e),'rb') as f:
      ev=pickle.load(f)
    label={0:0,1:0,2:0}   
    # for event in ev:
    for tree in ev.treelist:
...
def train_loader(val_index):
  for e in [l for i,l in enumerate(event_labels) if i!=val_index]:
    with open('data/Encoding/SKP/data_UnlimitedTree_{}_balanced.pkl'.format(e),'rb') as f:
      ev=pickle.load(f)
    yield ev,e

def val_loader(val_index):
  # for e in [l for i,l in enumerate(event_labels) if i==val_index]:
  with open('data/Encoding/SKP/data_UnlimitedTree_{}.pkl'\
      ...
      for indx,tree in enumerate(event.treelist):
        ...
        for i in range(len(nodes)):
          if(i!=(len(nodes)-1)):
            ...
            loss+=criterion(result,torch.tensor(veracity_dict[str(tree[0])],dtype=torch.long).view(-1))         
        # if(stance_count>0):
        #   stance_loss=stance_loss/stance_count
